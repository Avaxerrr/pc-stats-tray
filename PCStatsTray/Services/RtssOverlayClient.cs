...
namespace PCStatsTray
{
    internal sealed class RtssOverlayClient : IDisposable
    {
        ...
        private const string OwnerId = "PCStatsTray";
        ...
        public bool Release()
        {
            try
            {
                ...
                {
                    RecordPushResult(false, "No PC Stats Tray RTSS OSD slot was owned.", 0);
                }
        ...
        private static string ComposeStatus(RtssStatusSnapshot snapshot, string? overrideError = null)
        {
            ...
            lines.AppendLine(snapshot.IsSlotOwned
                ? "PC Stats Tray slot: acquired"
                : "PC Stats Tray slot: not acquired");
            ...
            {
                lines.AppendLine("PC Stats Tray has not captured an RTSS OSD slot yet.");
            }
